---
page_title: "extract_id function - nullplatform"
subcategory: ""
description: |-
...
Use this function when the entity type comes from a variable; when it is fixed, the dedicated variants (`extract_account_id`, `extract_namespace_id`, etc.) are more direct.
...
locals {
  ...
  account_id   = provider::nullplatform::extract_id("account", var.namespace_nrn)
  namespace_id = provider::nullplatform::extract_id("namespace", var.namespace_nrn)
}
```
...
```text
extract_id(entity_type string, nrn string) string
```
