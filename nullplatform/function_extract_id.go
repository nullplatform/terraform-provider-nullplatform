...
// extractIDFunction implements provider::nullplatform::extract_id, taking the
// entity type as its first argument for cases where the type comes from a
// variable. The extract_<entity>_id variants cover the common fixed-type case.
...
// extractEntityIDFunction implements the extract_<entity>_id family of
...
func (f *extractEntityIDFunction) Metadata(_ context.Context, _ function.MetadataRequest, resp *function.MetadataResponse) {
	resp.Name = fmt.Sprintf("extract_%s_id", f.entityType)
}
